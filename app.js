...
const levelBtnElement = document.getElementById("level-btn");
const modeBtnElement = document.getElementById("mode-btn");
const levelDropdownElement = document.getElementById("level-dropdown");
const modeDropdownElement = document.getElementById("mode-dropdown");
const startTypingContainer = document.getElementById("start-typing-container");
const wpmElement = document.getElementById("wpm-stat");
const accuracyElement = document.getElementById("accuracy-stat");
const timeElement = document.getElementById("time-stat");
const levelArrow = document.getElementById("level-arrow");
const modeArrow = document.getElementById("mode-arrow");
const levelList = document.querySelectorAll(`input[name="level"]`);
const modeList = document.querySelectorAll(`input[name="mode"]`);
...
// CLASS
class game {
	constructor(wpm, accuracy, time, level, mode){
		this.wpm = wpm;
...
// LOCAL STORAGE AND GLOBAL VARIABLE
const data = localStorage.getItem('typing-data');
let level = 'easy';
let mode = 'Timed (60s)';
let currentTextId = 0;
let wpm;
...
let gameLevel = level;
let gameMode = mode;
let levelText = getRandomLevelText(level);
let textArr = levelText.split("");
let out = [...textArr];
...
if (data === null) {
	currentData = new scoreBoard();
	initialisationWithNoData();
} else {
	currentData = JSON.parse(data);
	initialisationWithData();
}


// EVENTLISTENER
levelBtnElement.addEventListener('click',showHideLevelDropdown);
modeBtnElement.addEventListener('click',showHideModeDropdown);
levelList.forEach(e=>{
	e.addEventListener('click', ()=>updateLevel(e));
});
modeList.forEach(e=>{
	e.addEventListener('click', ()=>updateMode(e));
});
startTypingContainer.addEventListener('click', startAGame);
restartBtn.addEventListener('click', restartVariableAndShowStartScreen)


// FUNCTIONS
function initialisationWithNoData() {
	bestScoreElement.textContent = "0 WPM";
	wpmElement.textContent = "0";
	accuracyElement.textContent = "0%";
	timeElement.textContent = "0:00";
}

function initialisationWithData() {
	const lastGame = currentData.games[currentData.length - 1];
	bestScoreElement.textContent = `${currentData.bestGame.wpm} WPM`;
	wpmElement.textContent = `${lastGame.wpm}`;
	accuracyElement.textContent = `${lastGame.accuracy}%`;
	timeElement.textContent = `${lastGame.time}`;
}

function showHideLevelDropdown(){
	if (levelBtnElement.classList.contains('active')){
		levelDropdownElement.style.maxHeight = 0;
		levelDropdownElement.style.overflow = "hidden";
		levelArrow.style.transform = "rotate(360deg)"
	} else {
		levelDropdownElement.style.maxHeight = "fit-content";
		levelDropdownElement.style.overflow = "visible";
		levelArrow.style.transform = "rotate(180deg)"
	}
	levelBtnElement.classList.toggle('active');
}

function showHideModeDropdown (){
	if (modeBtnElement.classList.contains('active')){
		modeDropdownElement.style.maxHeight = 0;
		modeDropdownElement.style.overflow = "hidden";
		modeArrow.style.transform = "rotate(360deg)"
	} else {
		modeDropdownElement.style.maxHeight = "fit-content";
		modeDropdownElement.style.overflow = "visible";
		modeArrow.style.transform = "rotate(180deg)"
	}
	modeBtnElement.classList.toggle('active');
}

function updateLevel(e) {
	level = e.value;
	levelELement.textContent = level.slice(0,1)[0].toUpperCase() + level.slice(1);
}
function updateMode(e) {
	console.log('inside mode')
	mode = e.value;
	modeELement.textContent = mode.slice(0,1)[0].toUpperCase() + mode.slice(1);
}

function hideDropdown() {
		levelDropdownElement.style.maxHeight = 0;
		levelDropdownElement.style.overflow = "hidden";
		levelArrow.style.transform = "rotate(0)"
		modeDropdownElement.style.maxHeight = 0;
		modeDropdownElement.style.overflow = "hidden";
		modeArrow.style.transform = "rotate(0)"
}

function startAGame() {
	hideDropdown();
	initializeTextArrInHTML(textArr);
	keydownHandler = (key) => handleKey(key, textArr);
	document.addEventListener('keydown',keydownHandler);

	startTypingContainer.style.display = "none";
}

function removeEventListenerFortyping() {
	document.removeEventListener('keydown',keydownHandler);
}

function getRandomLevelText(level) {
	const currentLevel = levelsData[level];
	const randNum = Math.floor(Math.random() * 9);
	return currentLevel[randNum].text;
}

function initializeTextArrInHTML(textArr) {
	let out = [];
		out.push(currentText(textArr[0]))
	textContainer.innerHTML = out.join("") + textArr.slice(1).join("");
}


function greenText(char) {
	return `<span class="green-text">${char}</span>`
};

function redText(char) {
	return `<span class="red-text">${char}</span>`
};

function currentText(char) {
	return `<span class="current-char">${char}</span>`
};

function handleKey(key, textArr) {
		if (currentTextId === textArr.length) {
			removeEventListenerFortyping();
			showResult();
			return
		}
		const currKey = key.key;
		if(currKey.length > 1) return;
		console.log(`Curr key: ${currKey} | currtext: ${textArr[currentTextId]}` )
		if (currKey === textArr[currentTextId]) {
			out[currentTextId] = greenText(currKey);
		} else {
			out[currentTextId] = redText(textArr[currentTextId]);
			console.log(out[currentTextId]);
		}
		currentTextId++;
			if(currentTextId < textArr.length){
				out[currentTextId] = currentText(textArr[currentTextId]);
			}
		textContainer.innerHTML = out.join("");
}

function restartVariableAndShowStartScreen() {
	currentTextId = 0;
	levelText = getRandomLevelText(level);
	textArr = levelText.split("");
	out = [...textArr];
	startTypingContainer.style.display = "flex";
}
